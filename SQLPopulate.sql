...
-- -----------------------------------------------------
-- Table `utopia`.`booking_counter`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `utopia`.`booking_counter` (
  `booking_id` INT UNSIGNED NOT NULL,
  `counter_id` INT UNSIGNED NOT NULL,
  INDEX `fk_booking_booker_user1_idx` (`counter_id` ASC) VISIBLE,
  PRIMARY KEY (`booking_id`),
  UNIQUE INDEX `booking_id_UNIQUE` (`booking_id` ASC) VISIBLE,
  CONSTRAINT `fk_booking_booker_user10`
    FOREIGN KEY (`counter_id`)
    REFERENCES `utopia`.`user` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_booking_booker_booking10`
    FOREIGN KEY (`booking_id`)
    REFERENCES `utopia`.`booking` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
